...
namespace bof {
    ...
    namespace output {
        ...
        struct ReturnData {
            std::vector<bof::output::OutputEntry> outputBuf;
            int returnVal;
        };
     ...
     */
    template <typename... T>
    struct bof::output::ReturnData runMocked(int (*entry)(char*, int), T &&...v) {
        //Create struct to hold data
        struct bof::output::ReturnData retData;
        ...
        retData.returnVal = entry(args.get(), args.size());
        // Return the stored outputs
        retData.outputBuf = bof::output::getOutputs();
        return retData;
    }
}